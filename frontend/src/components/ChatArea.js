...
//   await onSessionUpdate(session.sessionId);
// };
...
import Message, { TypingIndicator } from './Message';
import UploadPanel from './UploadPanel';
import { askQuestion, clearMessages ,  getSession } from '../utils/api';
      ...
      <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 8, width: '100%', maxWidth: 320 }}>
        {['What is this document about?', 'Summarize the key points', 'What are the main conclusions?'].map(q => (
          <div key={q} style={{
            padding: '8px 14px', borderRadius: 'var(--radius-sm)',
            background: 'var(--bg-elevated)', border: '1px solid var(--border)',
            fontSize: 13, color: 'var(--text-secondary)', cursor: 'default', textAlign: 'left'
          }}>💬 {q}</div>
        ))}
      </div>
    )}
...
export default function ChatArea({ session, onSessionUpdate }) {
  // ✅ Initialize directly from session prop — safe because App.js normalizes it
  const [messages, setMessages] = useState(session?.messages || []);
  const [input, setInput] = useState('');
  const [loading, setLoading] = useState(false);
  const [showUpload, setShowUpload] = useState(!(session?.files?.length > 0));
  const [files, setFiles] = useState(session?.files || []);
  const [showClearConfirm, setShowClearConfirm] = useState(false);
  const bottomRef = useRef();
  const textareaRef = useRef();

  // ✅ Because we use key={sessionId} in App.js, this component fully remounts
  // on session switch — so useState initializers above are enough.
  // This effect only needed for when session data updates WITHIN same session
  useEffect(() => {
    if (!session) return;
    setMessages(session.messages || []);
    setFiles(session.files || []);
    setShowUpload(!(session.files?.length > 0));
  }, [session?.sessionId]);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      {/* ✅ Upload panel — shows when no files OR user clicks "+ Add PDFs" */}
      {showUpload && (
        <div style={{
          padding: '16px 24px', borderBottom: '1px solid var(--border)',
          background: 'var(--bg-surface)', flexShrink: 0
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            <button onClick={() => setShowClearConfirm(false)} style={{
              ...
              background: '#4b5563', color: 'white', cursor: 'pointer', fontFamily: 'inherit'
            }}>Cancel</button>
      ...
      <div style={{
        ...
        <div style={{
          display: 'flex', gap: 10, alignItems: 'flex-end',
          background: 'var(--bg-elevated)', border: '1px solid var(--border)',
          borderRadius: 'var(--radius)', padding: '10px 12px', transition: 'border-color 0.15s',
        }}
          onFocus={e => e.currentTarget.style.borderColor = 'var(--accent)'}
          onBlur={e => e.currentTarget.style.borderColor = 'var(--border)'}
        >
          ...
          <button
            ...
            style={{
              width: 36, height: 36, borderRadius: 10, border: 'none', flexShrink: 0,
              background: input.trim() && hasFiles && !loading
                ? 'linear-gradient(135deg, var(--accent), #9c5fff)' : 'var(--bg-hover)',
              color: input.trim() && hasFiles && !loading ? 'white' : 'var(--text-muted)',
              cursor: input.trim() && hasFiles && !loading ? 'pointer' : 'not-allowed',
